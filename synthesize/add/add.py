...
def main(api_key, model_name, example, save_path, debug=False):
    ...
    while cur_retries < max_retries:
        try:
            ...
            input_prompt = find_new_constraints_prompt_en.replace("[[[background]]]", background).replace("[[[logic_constraints]]]", logic_constraints).replace("[[[solution_space]]]",repr(solution_space)).replace("[[[Constraint_List_code]]]", Constraint_List_code).strip()
...
if __name__ == "__main__":
    ...
    input_path = f"./synthesize/add/output_en/{args.name}_prepocessed.jsonl"
    save_path = f"./synthesize/add/output_en/{args.name}_final.jsonl"
