    ...
    for subset in subsets:
        ...
        if len(logic_constraints_list) == nums_len and subset != []:
            ...
            while cur_retries < max_retries:
                try:
                    input_prompt = merge_constraints_prompt_en.replace("[[[origin_constraints_description]]]", logic_constraints_raw).replace("[[[Constraint_List_code]]]", temp['Constraint_List_code'])
...
if __name__ == "__main__":
    ...
    input_path = f"./synthesize/traverse/output_en/{args.name}_final.jsonl"
    save_path = f"./synthesize/delete/output_en/{args.name}_final.jsonl"
