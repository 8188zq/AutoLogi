...
def format_verify_prompt(data):
    prompt = build_verify_function_prompt_en
...
if __name__ == "__main__":
    ...
    if verify_turn_id == 1:
        ...
        use_cache = True
    input_path = f"./synthesize/verify/output_en/{args.name}.jsonl"
    save_path = f"./synthesize/verify/output_en/{args.name}_v{verify_turn_id}.{traverse_turn_id}.jsonl"
    ...
    if args.debug:
        ...
        if use_cache and verify_turn_id>1:
            cache_path = f"./synthesize/traverse/output_en/{args.name}_v{verify_turn_id-1}.{args.max_traverse_time}.jsonl"
